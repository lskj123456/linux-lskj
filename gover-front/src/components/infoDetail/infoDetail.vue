...
<!-- 	<p class="bread">
	...
	<div class="edit">
		<div>
			<span class="sty">{{time}}</span>
			<span class="sty">来源：经济日报</span>
		</div>
		<div>
			<span class="sty">
				<span>【 字体：</span>
				<span class="zt" @click="getFontsize('a')">大</span>
				<span class="zt" @click="getFontsize('b')">中</span>
				<span class="zt" @click="getFontsize('c')">小</span>
				<span>】</span>
			</span>
	...
	<div class="contain">
		<p v-html="conts" :style="{fontSize:size+'px'}"></p>
...
export default{
	data(){
		return {
			...
			conts:null || "hahahahahaahahahaahaahahaha" ,
			name: null || "测试",
			title: null || "hahaha",
				 // 视频播放
	    //   playerOptions : {
	...
	methods:{
		getMes(){
			this.$axios.post('/leading/selectByPrimaryKey',this.$qs.stringify({allmsgId:this.id})).then(res => {
				//console.log(res)
				this.conts = res.data.data.allmsgContents
				this.title = res.data.data.allmsgTitle
				this.name = res.data.data.allmsgCategoryName
				this.time = res.data.data.allmsgCreatetime
				console.log(res)
			})
		},
		getFontsize(val){
